...
class Optimizer(Engine):
    ...
    ----------
    variant : str or None
        Name of the optimizer variant. Defaults are specific to the optimization method used.
    max_iterations : int
    ...
    sampler : str
        Method for initializing starting points for the optimization. If ``'random'`` a uniform random generator
        is used. If ``'halton'``, ``'sobol'``, or ``'lhs'``, then Halton, Sobol, or LatinHypercube sampler is used,
        all of which should cover the domain more evenly. Default: ``'random'``.
    _sampler_method : Halton, Sobol, LatinHypercube, or None
    ...
    _progress_bar : Progress
        Private rich Progress object used for dashboard monitoring.
    _progress_bar_tasks : list
        Private list of rich progress bar task identifiers used for dashboard monitoring.
    _live : Live
        Private rich Live object used for dashboard monitoring.
    ...
    forward_unique_str : bool
        ...
        passed on to **evaluator** as an additional argument. Default: ``False``.
    ...
    safe_evaluation : bool
        If ``True``, in case of failed evaluations exceptions thrown by the **evaluator**
        are caught and ``np.nan`` is returned instead. Default: ``False``.
    eval_fail_count : int
        Used for counting failed evaluations.
    eval_fail_behavior : str
        Controlling behavior in case of **evaluator** returning ``np.nan``.
    ...
    def _init_optimizer(self):
        ...
        if not self.safe_evaluation:
            self._evaluator_safe = self.evaluator
        ...
        if self.sampler:
            assert self.sampler in 'random halton sobol lhs'.split(), \
                "optimizer.sampler should be 'random', 'halton', 'sobol', or 'lhs'"
        else:
            self.sampler = 'random'
    ...
    def _randomize_categorical(self, candidates: list[Candidate]) -> None:
        ...
        ----------
        candidates : list of Candidate
            Candidates whose categorical variables should be randomized.
    ...
    def optimize(self, resume=False, inject=None, seed=None):
        ...
        # Cast the best candidate to Candidate class
        r_c = Candidate(self.variables)
    ...
    def _multiprocess_evaluate(self, candidates: list[Candidate]):
        ...
        if self.forward_unique_str:
            ...
            results = self._pool.starmap(self._evaluator_safe,
                                         [(c.X, c.unique_str) for c in candidates])
        else:
            results = self._pool.map(self._evaluator_safe,
            ...
            if self.eval_fail_behavior == 'ignore':
                ...
                for p in nans_index:
                    for i in range(self.eval_retry_attempts):
                        ...
                        # re-evaluate
                        result_p = self._evaluator_safe(candidates[p].X)
    ...
    def _collective_evaluation(self, candidates: list[Candidate]):
        ...
        if self.processes > 1:
            ...
            for p in range(n):
                ...
                if self.forward_unique_str:
                    candidates[p].unique_str = self._gen_unique_str()
                    result = self._evaluator_safe(candidates[p].X, candidates[p].unique_str)
                else:
                    result = self._evaluator_safe(candidates[p].X)
    ...
    def plot_history(self, filename=None, title=None):
        ...
        if ax_o:
            ...
            ax_o.set_ylabel(r'$\mathbf{Objectives}$\n(lin scale)')
        ...
        if ax_f:
            ...
            ax_f.set_ylabel(r'$\mathbf{Fitness}$\n(norm log scale)')
